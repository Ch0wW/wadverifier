...
func CheckIWAD(filename string, hash string) {
	...
	if (bFound) {
		...
		fmt.Println("Version:", IWAD.Version)
		bNeedsPatching = !IWAD.IsFinal;
		fmt.Println("Needs patching ?", YesorNo(bNeedsPatching))
		fmt.Println("")
	} else {
		...
		fmt.Println(filename," seems unknown. Make sure it's not a modified file or not a PWAD file. (hash:",hash,")")
		fmt.Println("")
	}
}